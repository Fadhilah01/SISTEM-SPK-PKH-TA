 ...
 * Handles admin #hapusModal AND calon #hapusCalonModal:
 * sets form action and body text from the triggering button's data-id and data-nama.
 */
function initDeleteModal() {
    // Admin user delete modal
    const adminModal = document.getElementById('hapusModal');
    if (adminModal) {
        adminModal.addEventListener('show.bs.modal', function (event) {
            ...
            const bodyEl = adminModal.querySelector('#hapusModalBody');
            const formEl = adminModal.querySelector('#hapusForm');
    ...
    // Calon delete modal
    const calonModal = document.getElementById('hapusCalonModal');
    if (calonModal) {
        calonModal.addEventListener('show.bs.modal', function (event) {
            const btn = event.relatedTarget;
            if (!btn) return;
            ...
            const bodyEl = calonModal.querySelector('#hapusCalonModalBody');
            const formEl = calonModal.querySelector('#hapusCalonForm');

            if (bodyEl) {
                bodyEl.textContent = 'Hapus data calon ' + nama + '? Semua data terkait akan dihapus permanen.';
            }
            if (formEl) {
